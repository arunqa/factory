  ...
  return `<footer class="footer font-size-medium" style="position: absolute; bottom: 0; height: 60px; margin-top: 40px; width: 100%; display:none;">
    <div class="slds-container_x-large slds-container_center slds-p-left_small slds-p-right_small">      
      <article class="slds-text-align_center slds-p-top_small slds-p-bottom_large">
        ...
        <p class="slds-text-body_small">Publication created <script>document.write(getTimeAgo(new Date('${layout.dsArgs.renderedAt}')))</script></p>
        <p class="slds-text-body_small">${layout.dsArgs.git ? ` 🌲 ${layout.dsArgs.git?.cached.currentBranch || "??"}` : "not in Git work tree"} 📄 Modified <script>document.write(getTimeAgo(new Date('${layout.activeRoute?.terminal?.lastModifiedAt}')))</script></p>
      </article>
    </div>
  </footer>`;
};
