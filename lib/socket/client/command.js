...
class EventEmitterSocketCommandsHandler {
  constructor(element = document, cmdEventPrefix = "") {
    this.onMessage = (event, socketCmdsManager) => {
      const message = JSON.parse(event.data);
      if (message && message.nature == "command") {
        const commandID = message.identity;
        if (this.onShouldCommandBeHandled) {
          ...
          const eventID = `${cmdEventPrefix}${commandID}`;
          element.dispatchEvent(new CustomEvent(eventID, {
            detail: { nature: 'command', commandID, commandArgs: message.arguments, socketEvent: event, socketCmdsManager }
          }))
        }
        if (this.onAfterCommandHandled) {
          this.onAfterCommandHandled(commandID, message.arguments, handlerResult, handler, message, event, socketCmdsManager);
...
function socketCmdsMgrVisualsProseDecorator(socketCmdsManager, options = {
  ...
  scmStrategy.onConnEstablished = (event, scm, attemptNum, reestablishing) => {
    ...
    updateConnStateProse(`Connection ${reestablishing ? 'reestablished' : 'established'} for <code><a href="${scm.userAgent.locationURL}">${scm.userAgent.locationURL}</a></code>`);
