...
function socketCmdsMgrVisualsProseDecorator(socketCmdsManager, options = {
  ...
  scmStrategy.onConnEstablished = (event, scm, attemptNum, reestablishing) => {
    ...
    updateConnStateProse(`Live client diagnostics connection ${reestablishing ? 'reestablished' : 'established'} for <code>${scm.userAgent.locationURL}</code>.<br/><code>${scm.socketEndpointURL}</code><p>${new Date()}.${attemptNum > 1 ? `<br/>${attemptNum} attempts.</p>` : ''}`);
