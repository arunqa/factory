...
function eventSourceConnlNarrative(tunnel, reconn) {
...
export { eventSourceConnlNarrative as eventSourceConnlNarrative };
...
function badgenLiveBlock(lbInit) {
    const { badgenBlockSupplier , badgeElementSupplier , init , renderBadgeContentEventName ="render-badge-content" ,  } = lbInit;
    const badgeElement = badgeElementSupplier(lbInit);
    badgeElement.addEventListener(renderBadgeContentEventName, (event)=>{
        const { badgenBlock: badgenBlock1 = badgeElement.badgenBlock , content , display  } = event.detail;
        if (badgenBlock1) {
    ...
    const block = {
        ...
        content: (set, display)=>{
            if (set) {
                content1 = set;
                if (badgeElement) {
                    badgeElement.dispatchEvent(new CustomEvent(renderBadgeContentEventName, {
                        detail: {
                            content: content1,
                            display
                        }
            ...
            console.log("[badgenLiveBlock] content", set, content1);
            return content1;
        },
        display: (state)=>{
            if (badgeElement) {
